...
erreur <- predict_xgb_1_6 %>% bind_cols(auc_test_1_6) %>% mutate(diff= auc...2-.pred)
summary(erreur)

erreur %>% filter(diff>20)
...
base_externe_finalIV3bis <- base_externe_finalIV3 %>% mutate(auc_bis=case_when(auc_24<70~auc_24,auc_24>=70~ auc_24/2))
summary(base_externe_finalIV3bis)
base_externe_finalIV4<-base_externe_finalIV3bis%>% mutate(SEX1=case_when(SEX=="0"~"garcon",SEX=="1"~"fille"))%>% select(-SEX)%>% rename(SEX=SEX1) %>% rename(auc=auc_bis)
...
#### performance sans les ID 5a,36a,59, 52,60

base_externe_finalIV_long2_filter <- base_externe_finalIV_long2  %>% filter(!ID=="5a") %>% filter(!ID=="36a") %>% filter(!ID=="59")  %>% filter(!ID=="52") %>% filter(!ID=="60")


predict_baseextIV_1_6_filter=predict(final_wf_xgb_1_6,base_externe_finalIV_long2_filter)%>%
     bind_cols(auc=base_externe_finalIV_long2_filter$auc) %>%
  mutate (bias_rel = (.pred - auc)/auc,
          bias_rel_square = bias_rel * bias_rel)



rmarkdown::paged_table(as.data.frame(predict_baseextIV_1_6_filter %>% summarise(biais_rel = mean(bias_rel), relative_rmse = sqrt(mean(bias_rel_square)),biais_out_20percent = mean(!between(bias_rel,-0.2, 0.2)), nb_out_20percent = sum(!between(bias_rel,-0.2, 0.2)),biais_out_10percent = mean(!between(bias_rel,-0.1, 0.1)), nb_out_10percent = sum(!between(bias_rel,-0.1, 0.1)), n= n())))
