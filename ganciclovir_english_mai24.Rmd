...
#### we separated into 2 datasets  for simulations with Franck and nguyen models

set.seed(123)  
sample <- sample.int(n = nrow(Simu_totale_avec_dose2), size = floor(.5*nrow(Simu_totale_avec_dose2)), replace = F)

data_bene <- Simu_totale_avec_dose2[sample, ]
data_nguyen  <- Simu_totale_avec_dose2[-sample, ]
...
data_globale <-data_bene2 %>% bind_rows(data_nguyen2)  
...
e_bene <- expand.ev( ii = 24, ID=1:5400 ,addl = 5,ss=1,tinf = 1)  %>% bind_cols(data_bene)%>% mutate(ID=1:5400 )%>%  select(-ID...16,-amt...3,-ID...16,-ID...1)%>% rename(amt=amt...19)
summary(e_bene)
...
e_nguyen <- expand.ev( ii = 24, ID=1:5400 ,addl = 5,ss=1,tinf=1) %>% bind_cols(data_nguyen)%>% mutate(ID=1:5400 )%>%  select(-ID...1,-amt...3,-ID...16)%>% rename(amt=amt...19) 
...
simu_auc <- simu_auc_bene%>% bind_rows(simu_auc_nguyen)
...
auc <- auc %>%  filter(auc <= 120 & auc>=6 ) %>% mutate(dose_mg=amt/Pds)
...
set.seed(1234)
auc_1<- auc %>% dplyr::select( -ID,-dose_mg)
summary(auc_1)

saveRDS(auc_1,"auc1.rds")
...
```
 ...
 0/2 Xgboost had the best performances, we develop on base_train 
...
auc_rec_0_2 <- recipe(auc ~ ., data = auc_train_0_2) %>%
  step_normalize(all_numeric(), -all_outcomes()) %>%
  step_dummy(greffe,SEX,one_hot=T) 
...
##hyperparameters
set.seed(2345)
auc_folds_0_2 <- vfold_cv(auc_train_0_2)#par défaut 10 fois
...
###resample#####
set.seed(456)
folds_0_2 <- vfold_cv(auc_train_0_2)#par défaut 10 fois
set.seed(123)
xgb_rs_0_2 <- fit_resamples(object = final_wf_xgb_0_2, resamples = folds_0_2, control = control_resamples(verbose=T, save_pred = T))
...
summary(auc_test_0_2)
predict_xgb_0_2=predict(final_wf_xgb_0_2,auc_test_0_2)%>%
     bind_cols(auc=auc_test_0_2$auc) %>%
  mutate (bias_rel = (.pred - auc)/auc,
          bias_rel_square = bias_rel * bias_rel)
